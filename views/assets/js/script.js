...
function colisioDetectada(player, ball) {
  // Posició i mesura de la pala
  player.top = player.y;
  player.right = player.x + player.width;
  player.bottom = player.y + player.height;
  player.left = player.x;

  // Posició i mesura de la bola
  ball.top = ball.y - ball.radius;
  ball.right = ball.x + ball.radius;
  ball.bottom = ball.y + ball.radius;
  ball.left = ball.x - ball.radius;

  // Retorna si la bola ha xocat contra la pala
...
function update() {
  ...
  // Quan es colisiona es determina quin jugador ha rebut la colisió. Si és a la zona esquerra seria la pala 1, i si és a la zona de la dreta seria la pala 2.
...
function gameLoop() {
  ...
  // Aquesta variable guardarà el missatge de victòria
  let guanyador;
